  ...
  @keyframes sk-cubemove {
    ...
    <div class="cube2"></div>
      ...
      response.write(`
        ...
        var gens = [
          {
            name: "Királyi magyarország részei",
            choose: [
              {
                text: "nyugat, kelet",
                isCorrect: false
          ...
          },
          {
            ...
            choose: [
              ...
              {
                text: "egy híres művész",
                isCorrect: false
              },
              {
                text: "Erdély vezetője",
                isCorrect: true
              },
              {
                text: "nemtom",
        ...
        for (let i = 0; i < gens.length; i++) {
          let dux = ""
          for (let is = 0; is < gens[i].choose.length; is++) {
            {
              let us = is
              dux = dux+'<button onclick="if(gens['+i+'].choose['+us+'].isCorrect){alert(\\'Helyes!\\'); oobeStep++; refreshObe(); closeWindow(getWindowBytitle(\\'virtOS setup - \\'+OOBE[oobeStep-1].name).id) }else{alert(\\'Helytelen! Most nomyhatod a kvizt elölrő!! XDDDD\\'); let obe = oobeStep; oobeStep = 0; refreshObe(); closeWindow(getWindowBytitle(\\'virtOS setup - \\'+OOBE[obe-1].name).id)}">'+gens[i].choose[us].text+'</button>'
            }

          }
          OOBE.push({
            name: gens[i].name,
            data: "Melyik a jó?<br>"+dux
          })
        }
        var oobeStep = 0

        function refreshObe() {
          addWindow("virtOS setup - "+OOBE[oobeStep].name,OOBE[oobeStep].data+'<button style="text-align: bottom, right;" onclick="oobeStep++; refreshObe(); closeWindow(getWindowBytitle(\\'virtOS setup - \\'+OOBE[oobeStep-1].name).id)">Kihagy \></button>',800,600)
        }
