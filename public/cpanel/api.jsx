...
exports.run = async function(request, response, args, fs) {
    ...
    if (ret == "true") {
        await fetch("http://localhost:1234/token_login/api.jsx?token=" + args.token + "&webtoken=ghjdfsagozdsafgihdsafbhdsafhljbgdsafgdsafhgdsafgdaf&mode=protData&key=cpanel_owned_servers")
        if (args.mode == "new") {
            ...
            reta = await fetch("http://localhost:1234/token_login/api.jsx?token=" + args.token + "&mode=getData&key=cpanel_owned_servers")
            ...
            if (ret != "err_token_invalid") {
                ...
                reta = await fetch("http://localhost:1234/token_login/api.jsx?token=" + args.token + "&mode=setData&webtoken=ghjdfsagozdsafgihdsafbhdsafhljbgdsafgdsafhgdsafgdaf&key=cpanel_owned_servers&data=" + JSON.stringify(dat))
                ret = await reta.text()
        ...
        if (args.mode == "getInfo") {
            ...
            if (fs.existsSync("./cpanel/servers/" + args.id + "/data.json")) {
                ...
                let sere = await fetch("http://localhost:1234/token_login/api.jsx?token=" + args.token + "&mode=getData&key=cpanel_owned_servers")
