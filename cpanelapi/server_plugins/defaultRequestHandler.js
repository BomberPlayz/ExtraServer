...
exports.onStart = function(server,logger) {
    ...
    setInterval(function() {
        for (let lit in server.rateLimited) {
            ...
            if (server.rateLimited[lit] >= 1) {
                server.rateLimited[lit] =server.rateLimited[lit]-1
            } else {
    ...
    let ipban = ["::ffff:69.4.89.106","::ffff:37.252.5.88","::ffff:93.119.227.19",/*"::ffff:89.147.71.64"*/]
    ...
    server.requestHandler = function(request, response) {
      try {
        logger.info(request.connection.remoteAddress+" requested to "+request.url)
          ...
          console.log(server.rateLimited)
          if (typeof server.rateLimited[request.connection.remoteAddress] == "undefined") {
              console.log(request.connection.remoteAddress+" :: GEN RLIMIT")
              server.rateLimited[request.connection.remoteAddress] = 0
          } else {
              console.log("BEFORE::"+server.rateLimited[request.connection.remoteAddress])
              server.rateLimited[request.connection.remoteAddress] = server.rateLimited[request.connection.remoteAddress] + 0.1
              console.log("AFTER::"+server.rateLimited[request.connection.remoteAddress])
          }
          console.log(request.connection.remoteAddress+" RATELIMIT COUNT: "+server.rateLimited[request.connection.remoteAddress])
          if (server.rateLimited[request.connection.remoteAddress] > server.rateLimit && request.connection.remoteAddress != "::ffff:127.0.0.1") {
              console.log(request.connection.remoteAddress+" has been rateLimited!")
