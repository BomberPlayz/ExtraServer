exports.run = async (request, response, ip, args, fs) => {
  ...
  <div class="modal-dialog">
    ...
    async function modifyMulti() {
        ...
        await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode=setWattsMulti&value="+pat.value)
        pat.value = ""
    }
        ...
        auto()
            function auto() {
                setTimeout(async function () {
                    ...
                    try {
                        console.log("teried???")
                        let macska = await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode=getSpeed")
                let spem = await macska.text()
                
                let asdasdasdaaaaa = await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode=getWattsMulti")
                ...
                let macskar = await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode=getTorta")
                ...
                let macskart = await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode=getVolt")
                let sperma = await macskart.text()
                voltgag.set(Number(sperma)*4200);
                
                let togirt = document.getElementById('volttext');
                
                let macskarta = await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode=getWatt")
                ...
                let asdasdasda = await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode=getStarter")
                let starter = await asdasdasda.text()
                
                let asdasdasdasd = await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode=getGrid")
                let grid = await asdasdasdasd.text()
                
                let asdasdasdara = await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode=getGens")
                let gens = await asdasdasdara.text()
                
                let asdasdasd = await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode=getStop")
                let stop = await asdasdasd.text()
                
                let asdasdasdaaa = await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode=getPower")
                ...
                for (let i=0; i<stats.length; i++) {
                    ...
                    if (stats[i] == "on") {
                        ...
                        elem.onclick = async function(){console.log("ONNN_______"+maks[i]);await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode="+paks[i]+"&value=off")}
                    } else {
                        elem.setAttribute("class","btn btn-success")
                        elem.onclick = async function(){console.log("OF____________"+maks[i]);await fetch("http://win.crackpixel.hu:1026/api"+window.location.search+"&mode="+paks[i]+"&value=on")}
                    }
