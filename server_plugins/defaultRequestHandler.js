      ...
      try {
        logger.info(request.connection.remoteAddress+" requested to "+request.url)
          ...
          if (typeof server.rateLimited[request.connection.remoteAddress] == "undefined") {
              ...
              server.rateLimited[request.connection.remoteAddress] = server.rateLimited[request.connection.remoteAddress] + 0.05
        ...
        if (fs.lstatSync("." + request.urla).isDirectory()) {
            ...
            if (fs.lstatSync("." + request.urla).isFile()) {
                ...
                if (request.headers['range']) {
                    ...
                    server.rateLimited[request.connection.remoteAddress] = server.rateLimited[request.connection.remoteAddress] + 1.5
                    file.pipe(response);
